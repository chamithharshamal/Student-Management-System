...
@push('modals')
    ...
    {{-- Course Import Modal --}}
    <div id="courseImportModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/40 px-4 py-6 backdrop-blur-sm">
        <div class="w-full max-w-md overflow-hidden rounded-[28px] bg-white ring-1 ring-slate-200/50 shadow-[0_24px_80px_-12px_rgba(18,38,104,0.4)]">
            <div class="border-b border-slate-100 px-6 py-5 flex items-start justify-between gap-4">
                ...
                <button type="button" onclick="closeCourseImportModal()" class="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700">
                    <svg viewBox="0 0 24 24" class="h-5 w-5 fill-none stroke-current stroke-[2]"><path stroke-linecap="round" d="M6 6l12 12M18 6L6 18" /></svg>
                </button>
            </div>
            <form method="POST" action="{{ route('admin.courses.import') }}" enctype="multipart/form-data" class="px-6 py-6 space-y-5">
                @csrf
                <div class="rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 px-6 py-8 text-center">
                    ...
                    <label for="course_import_file" class="cursor-pointer text-sm font-semibold text-slate-700 hover:text-slate-900">
                        Click to choose file
                        <input id="course_import_file" type="file" name="file" accept=".csv,.xlsx,.xls" class="sr-only">
                    </label>
                    <p class="mt-1 text-xs text-slate-400">CSV, XLS, or XLSX · Max 10MB</p>
                </div>
                <p class="text-xs text-slate-400">Expected columns: <code class="bg-slate-100 px-1 rounded">code, name, description, credits</code></p>
                <div class="flex items-center gap-3">
                    <button type="button" onclick="closeCourseImportModal()" class="flex-1 rounded-3xl border border-slate-200 bg-white px-4 py-3.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">Cancel</button>
